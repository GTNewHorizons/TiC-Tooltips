...
public class ProxyIguanaTweaks {
    ...
    public static int getUnboostedHarvestLevel(Item item, int harvestLevel) {
        if (toolsRequireBoost && (item.getUnlocalizedName().toLowerCase(Locale.ROOT).contains("pick")
                || item.getUnlocalizedName().toLowerCase(Locale.ROOT).contains("hammer")))
            return Math.max(0, harvestLevel - 1);
        else return harvestLevel;
    }
}
