...
public class ToolPartHelper {
    ...
    public static List<Item> toolHeads = new ArrayList<Item>(
            Arrays.asList(TinkerTools.pickaxeHead, TinkerTools.shovelHead, TinkerTools.excavatorHead));
    public static List<Item> weaponMiningHeads = new ArrayList<Item>(Arrays.asList(TinkerTools.hammerHead));
    public static List<Item> weaponToolHeads = new ArrayList<Item>(
            Arrays.asList(TinkerTools.hatchetHead, TinkerTools.scytheBlade, TinkerTools.broadAxeHead));
    ...
    public static List<Item> weaponGuards = new ArrayList<Item>(
            Arrays.asList(TinkerTools.crossbar, TinkerTools.handGuard, TinkerTools.wideGuard));
    public static List<Item> fullWeaponGuards = new ArrayList<Item>(Arrays.asList(TinkerTools.fullGuard));
    public static List<Item> bindings = new ArrayList<Item>(Arrays.asList(TinkerTools.binding));
    public static List<Item> toughBindings = new ArrayList<Item>(Arrays.asList(TinkerTools.toughBinding));
    public static List<Item> rods = new ArrayList<Item>(Arrays.asList(TinkerTools.toolRod, TinkerTools.toughRod));
    public static List<Item> plates = new ArrayList<Item>(Arrays.asList(TinkerTools.largePlate));
    public static List<Item> shards = new ArrayList<Item>(Arrays.asList(TinkerTools.toolShard));
    public static List<Item> arrowHeads = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.arrowhead));
    public static List<Item> arrowFletchings = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.fletching));
    public static List<Item> bowStrings = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.bowstring));
    public static List<Item> arrowRods = new ArrayList<Item>(
            Arrays.asList(TinkerTools.toolRod, TinkerWeaponry.partArrowShaft));
    public static List<Item> chisels = new ArrayList<Item>(Arrays.asList(TinkerTools.chiselHead));
    // stuff added in 1.8.0
    public static List<Item> shurikenParts = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.partShuriken));
    public static List<Item> arrowShafts = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.partArrowShaft));
    public static List<Item> bowLimbs = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.partBowLimb));
    public static List<Item> crossbowLimbs = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.partCrossbowLimb));
    public static List<Item> crossbowBodies = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.partCrossbowBody));
    public static List<Item> boltParts = new ArrayList<Item>(Arrays.asList(TinkerWeaponry.partBolt));
    ...
    public static void determineMinAndMaxValues() {
        ...
        for (ToolMaterial mat : TConstructRegistry.toolMaterials.values()) {
            if (needsInit) {
                ...
                if (mat.shoddy() != 0f) {
                    if (mat.shoddy() > 0f && mat.shoddy() > maxPositiveShoddiness) maxPositiveShoddiness = mat.shoddy();
                    if (mat.shoddy() < 0f && mat.shoddy() < maxNegativeShoddiness) maxNegativeShoddiness = mat.shoddy();
                }
                if (mat.attack() > maxAttack) maxAttack = mat.attack();
                else if (mat.attack() < minAttack) minAttack = mat.attack();
                if (mat.harvestLevel() > maxHarvestLevel) maxHarvestLevel = mat.harvestLevel();
                else if (mat.harvestLevel() < minHarvestLevel) minHarvestLevel = mat.harvestLevel();
                if (mat.durability() > maxDurability) maxDurability = mat.durability();
                else if (mat.durability() < minDurability) minDurability = mat.durability();
                if (mat.toolSpeed() > maxMiningSpeed) maxMiningSpeed = mat.toolSpeed();
                else if (mat.toolSpeed() < minMiningSpeed) minMiningSpeed = mat.toolSpeed();
                if (mat.handleDurability() > maxHandleModifier) maxHandleModifier = mat.handleDurability();
                else if (mat.handleDurability() < minHandleModifier) minHandleModifier = mat.handleDurability();
                if (mat.reinforced() > maxReinforcedLevel) maxReinforcedLevel = mat.reinforced();
                else if (mat.reinforced() < minReinforcedLevel) minReinforcedLevel = mat.reinforced();
            }
        }
        ...
        for (ArrowMaterial mat : TConstructRegistry.arrowMaterials.values()) {
            if (needsInit) {
                ...
                if (mat.breakChance > maxBreakChance) maxBreakChance = mat.breakChance;
                else if (mat.breakChance < minBreakChance) minBreakChance = mat.breakChance;
                if (mat.mass > maxWeight) maxWeight = mat.mass;
                else if (mat.mass < minWeight) minWeight = mat.mass;
            }
        }
        ...
        for (BowMaterial mat : TConstructRegistry.bowMaterials.values()) {
            if (needsInit) {
                ...
                if (mat.drawspeed > maxBowDrawSpeed) maxBowDrawSpeed = mat.drawspeed;
                else if (mat.drawspeed < minBowDrawSpeed) minBowDrawSpeed = mat.drawspeed;
                if (mat.flightSpeedMax > maxFlightSpeedMax) maxFlightSpeedMax = mat.flightSpeedMax;
                else if (mat.flightSpeedMax < minFlightSpeedMax) minFlightSpeedMax = mat.flightSpeedMax;
                if (mat.flightSpeedMax > maxBowArrowSpeedModifier) maxBowArrowSpeedModifier = mat.flightSpeedMax;
                else if (mat.flightSpeedMax < minBowArrowSpeedModifier) minBowArrowSpeedModifier = mat.flightSpeedMax;
            }
        }
        ...
        for (CustomMaterial customMat : TConstructRegistry.customMaterials) {
            BowstringMaterial mat = (BowstringMaterial) TConstructRegistry
                    .getCustomMaterial(customMat.materialID, BowstringMaterial.class);
            if (mat != null) {
                if (needsInit) {
            ...
            if (fletchingMat != null) {
                if (fletchingMat.accuracy > maxAccuracy) maxAccuracy = fletchingMat.accuracy;
                else if (fletchingMat.accuracy < minAccuracy) minAccuracy = fletchingMat.accuracy;
                if (fletchingMat.breakChance > maxBreakChance) maxBreakChance = fletchingMat.breakChance;
                else if (fletchingMat.breakChance < minBreakChance) minBreakChance = fletchingMat.breakChance;
            ...
            ArrowShaftMaterial arrowShaftMat = (ArrowShaftMaterial) TConstructRegistry
                    .getCustomMaterial(customMat.materialID, ArrowShaftMaterial.class);
            if (arrowShaftMat != null) {
                ...
                if (arrowShaftMat.fragility > maxBreakChance) maxBreakChance = arrowShaftMat.fragility;
                else if (arrowShaftMat.fragility < minBreakChance) minBreakChance = arrowShaftMat.fragility;
                if (arrowShaftMat.weight > maxWeight) maxWeight = arrowShaftMat.weight;
                else if (arrowShaftMat.weight < minWeight) minWeight = arrowShaftMat.weight;
            }
        }
    }
    ...
    public static String getShoddinessString(float val) {
        return ColorHelper.getRelativeColor(val, minShoddiness, val > 0 ? maxPositiveShoddiness : maxNegativeShoddiness)
                + StringHelper.getShoddinessString(val);
    }
    ...
    public static String getHarvestLevelString(int val) {
        return ColorHelper.getRelativeColor(val, minHarvestLevel, maxHarvestLevel)
                + StringHelper.getHarvestLevelName(val);
    }
    ...
    public static String getHandleModifierString(float val) {
        return ColorHelper.getRelativeColor(val, minHandleModifier, maxHandleModifier)
                + StringHelper.getModifierString(val);
    }

    public static String getReinforcedString(int val) {
        return ColorHelper.getRelativeColor(val, minReinforcedLevel, maxReinforcedLevel)
                + StringHelper.getReinforcedString(val);
    }
    ...
    public static String getBowDrawSpeedString(int val) {
        return ColorHelper.getRelativeColor(val, maxBowDrawSpeed, minBowDrawSpeed)
                + StringHelper.getDrawSpeedString(val);
    }
    ...
    public static String getArrowSpeedString(float val, float multiplier) {
        return ColorHelper
                .getRelativeColor(val, minBowArrowSpeedModifier * multiplier, maxBowArrowSpeedModifier * multiplier)
                + StringHelper.getArrowSpeedString(val);
    }

    // bowstrings
    public static String getBowStringDrawspeedModifierString(float val) {
        return ColorHelper.getRelativeColor(val, minBowStringDrawspeedModifier, maxBowStringDrawspeedModifier)
                + StringHelper.getModifierString(val);
    }
    ...
    public static String getBowStringArrowSpeedModifierString(float val) {
        return ColorHelper.getRelativeColor(val, minBowStringArrowSpeedModifier, maxBowStringArrowSpeedModifier)
                + StringHelper.getModifierString(val);
    }

    public static String getBreakChanceString(float val) {
        return ColorHelper.getRelativeColor(val, maxBreakChance, minBreakChance)
                + StringHelper.getBreakChanceString(val);
    }

    public static String getDurabilityModifierString(float val) {
        return ColorHelper.getRelativeColor(val, minFletchingDurabilityModifier, maxFletchingDurabilityModifier)
                + StringHelper.getModifierString(val);
    }
