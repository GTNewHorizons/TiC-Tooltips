...
public class TooltipHandler {
    ...
    public void onItemTooltip(ItemTooltipEvent event) {
        ...
        if (item instanceof IToolPart) {
            if (ToolPartHelper.isShard(item)) return;
        ...
        // Tools
            ...
            if (event.toolTip.size() > 1) {
                ...
                // otherwise skip to the first + modifier or Max Damage line
                    ...
                    for (int toolTipSearchIndex = 0; toolTipSearchIndex < event.toolTip.size(); toolTipSearchIndex++) {
                        if (event.toolTip.get(toolTipSearchIndex).startsWith(plusPrefix)
                                || event.toolTip.get(toolTipSearchIndex).startsWith(maxDamagePrefix)) {
                ...
                if (toolTipIndex == -1) {
                    ...
                    toolTipIndex = Math
                            .max(
                                    0,
                                    (tinkersTooltip.size() > 0 ? Math.min(
                                            event.toolTip.size(),
                                            event.toolTip.indexOf(tinkersTooltip.get(0)) + tinkersTooltip.size())
            ...
            if (shiftDown && !ctrlDown) {
                if (toolTipIndex > 1) event.toolTip.add(toolTipIndex++, "");
            ...
            // Ctrl held
                ...
                while (event.toolTip.size() > 1) event.toolTip.remove(1);
            ...
            // No buttons held
                ...
                if (ModTiCTooltips.hasIguanaTweaks) event.toolTip.add(toolTipIndex, "");

                event.toolTip.add(
                        toolTipIndex++,
                        StatCollector.translateToLocalFormatted(
                                "tictooltips.hold.key.for.stats",
                ...
                event.toolTip.add(
                        toolTipIndex++,
                        StatCollector.translateToLocalFormatted(
                                "tictooltips.hold.key.for.materials",
    ...
    private List<String> getMaterialTooltip(int matID, Item item, ToolCore tool) {
        ...
        if (mat == null) return toolTip;

        if (!mat.ability().equals("")) toolTip.add(mat.style() + CompatibilityHelper.getLocalizedAbility(mat));
        ...
        if (mat.shoddy() != 0) {
            for (int index = 0; index < toolTip.size(); index++) {
                if (toolTip.get(index).contains(StringHelper.getShoddinessTypeString(mat.shoddy()))) {
                    toolTip.set(
                            index,
                            toolTip.get(index) + EnumChatFormatting.RESET
                                    + EnumChatFormatting.GRAY
                                    + StatCollector.translateToLocalFormatted(
                                            "tictooltips.material.shoddiness.modifier",
                                            ToolPartHelper.getShoddinessString(mat.shoddy()) + EnumChatFormatting.RESET
                                                    + EnumChatFormatting.GRAY));
                    break;
                }
            }
        ...
        if (ToolPartHelper.isArrowHead(item)) {
            toolTip.add(
                    StringHelper.getLocalizedString("gui.toolstation3") + ToolPartHelper.getAttackString(mat.attack()));
            toolTip.add(
            ...
            if (fletchingMat != null) {
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation2")
                                + ToolPartHelper.getDurabilityModifierString(fletchingMat.durabilityModifier));
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation22")
                                + ToolPartHelper.getBreakChanceString(fletchingMat.breakChance));
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation9")
                                + ToolPartHelper.getAccuracyString(fletchingMat.accuracy));
            }
        } else if (ToolPartHelper.isBowString(item)) {
            BowstringMaterial bowstringMat = (BowstringMaterial) TConstructRegistry
                    .getCustomMaterial(matID, BowstringMaterial.class);

            if (bowstringMat != null) {
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation6")
                                + ToolPartHelper.getBowStringDrawspeedModifierString(bowstringMat.drawspeedModifier));
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation2")
                                + ToolPartHelper.getBowStringDurabilityModifierString(bowstringMat.durabilityModifier));
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation7") + ToolPartHelper
                                .getBowStringArrowSpeedModifierString(bowstringMat.flightSpeedModifier));
            }
        } else {
            ...
            if (ToolPartHelper.isRod(item) || ToolPartHelper.isArrowShaft(item)) {
                ...
                boolean isArrowMat = arrowShaftMat != null && matID <= MaterialID.Wood
                        && ToolPartHelper.isArrowRod(item);
                boolean isToolAnArrow = hasTool && ToolHelper.isProjectile(tool) && ToolHelper.isAmmo(tool);
                ...
                if (isToolAnArrow || (isArrowMat && KeyHelper.isShiftKeyDown())) {
                    if (arrowShaftMat != null && (!hasTool || isToolAnArrow)) {
                        toolTip.add(
                                StringHelper.getLocalizedString("gui.toolstation2")
                                        + ToolPartHelper.getDurabilityModifierString(arrowShaftMat.durabilityModifier));
                        toolTip.add(
                                StringHelper.getLocalizedString("gui.toolstation22")
                                        + ToolPartHelper.getBreakChanceString(arrowShaftMat.fragility));
                        toolTip.add(
                                StringHelper.getLocalizedString("gui.toolstation8")
                                        + ToolPartHelper.getWeightString(arrowShaftMat.weight));
                    }
                } else {
                    String durabilityModifierTitle = StatCollector.translateToLocalFormatted(
                            "tictooltips.material.durability.modifier",
                            StringHelper.getLocalizedString("gui.toolstation2").replaceFirst(": ", ""));
                    toolTip.add(durabilityModifierTitle + ToolPartHelper.getHandleModifierString(mat.handleModifier));
                    if (!hasTool && isArrowMat) toolTip.add(
                            StatCollector.translateToLocalFormatted(
                                    "tictooltips.hold.key.for.bow.stats",
                                    EnumChatFormatting.YELLOW.toString() + EnumChatFormatting.ITALIC
                                            + "Shift"
                                            + EnumChatFormatting.RESET
                                            + EnumChatFormatting.GRAY));
                ...
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation2")
                                + ToolPartHelper.getDurabilityString(mat.durability()));
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation14")
                                + ToolPartHelper.getMiningSpeedString(mat.toolSpeed()));
                ...
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation15")
                                + ToolPartHelper.getHarvestLevelString(harvestLevel));
                ...
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation15")
                                + ToolPartHelper.getHarvestLevelString(mat.harvestLevel()));
                toolTip.add(
                ...
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation2")
                                + ToolPartHelper.getDurabilityString(mat.durability()));
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation16")
                                + ToolPartHelper.getMiningSpeedString(mat.toolSpeed()));
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation3")
                                + ToolPartHelper.getAttackString(mat.attack()));
                ...
                toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation2")
                                + ToolPartHelper.getDurabilityString(mat.durability()));
                if (!hasTool || ToolHelper.isHarvestTool(tool)) toolTip.add(
                        StringHelper.getLocalizedString("gui.toolstation14")
                                + ToolPartHelper.getMiningSpeedString(mat.toolSpeed()));
    ...
    private List<String> getPatternTooltip(ItemStack itemStack) {
        ...
        if (validMats != null && !validMats.isEmpty()) {
            ...
            for (String matName : validMats) {
                if (toolTip.size() < 7 || KeyHelper.isShiftKeyDown()) toolTip.add(" - " + matName);
                else {
                    toolTip.add(
                            StatCollector.translateToLocalFormatted(
                                    "tictooltips.hold.key.for.more",
                                    EnumChatFormatting.YELLOW.toString() + EnumChatFormatting.ITALIC
                                            + "Shift"
                                            + EnumChatFormatting.RESET
                                            + EnumChatFormatting.GRAY));
    ...
    public List<String> getDurabilityTooltip(NBTTagCompound toolTag, ItemStack itemStack) {
        ...
        if (maxDurability > 0) {
            ...
            String curOfMax = curDurability == maxDurability ? StringHelper.getDurabilityString(maxDurability)
                    : StringHelper.getDurabilityString(curDurability) + " / "
                            + StringHelper.getDurabilityString(maxDurability);
            toolTip.add(
                    StringHelper.getLocalizedString("gui.toolstation2")
                            + ColorHelper.getRelativeColor(curDurability, 0, maxDurability)
                            + curOfMax);

            if (maxDurability != effectiveDurability) {
                String effectiveDurabilityTitle = StatCollector.translateToLocalFormatted(
                        "tictooltips.tool.effective.durability",
                        StringHelper.getLocalizedString("gui.toolstation2"));
                String effectiveDurabilityColor = ColorHelper.getRelativeColor(
                        ToolHelper.getReinforcedLevel(toolTag),
                        ToolPartHelper.minReinforcedLevel - 3,
                        ToolPartHelper.maxReinforcedLevel);
    ...
    public List<String> getAmmoCountTooltip(IAmmo ammo, NBTTagCompound toolTag, ItemStack itemStack) {
        ...
        if (maxAmmo > 0) {
            ...
            String curOfMax = curAmmo == maxAmmo ? StringHelper.getAmmoCountString(maxAmmo)
                    : StringHelper.getAmmoCountString(curAmmo) + " / " + StringHelper.getAmmoCountString(maxAmmo);
            toolTip.add(
                    StringHelper.getLocalizedString("gui.toolstation21")
                            + ColorHelper.getRelativeColor(curAmmo, 0, maxAmmo)
                            + curOfMax);
        }
    ...
    public List<String> getWeaponTooltip(ToolCore tool, NBTTagCompound toolTag, float damageModifier) {
        ...
        float stoneboundDamage = ToolHelper.hasDurability(tool) ? ToolHelper.getShoddinessDamageBonus(tool, toolTag)
                : 0f;
        float maxStoneboundDamage = ToolHelper.hasDurability(tool)
                ? ToolHelper.getMaxShoddinessDamageBonus(tool, toolTag)
                : 0f;
        ...
        String damageColor = ColorHelper.getRelativeColor(
                ToolHelper.getRawDamage(tool, toolTag) + stoneboundDamage,
                ToolPartHelper.minAttack,
                ToolPartHelper.maxAttack);
        toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation3") + damageColor
                        + StringHelper.getDamageString(damage));

        // For ranged weapons (projectiles and thrown), the base damage is multiplied by the projectile speed to
        ...
        if (baseDamage >= 0) {
            String baseDamageColor = ColorHelper
                    .getRelativeColor(baseDamage, ToolPartHelper.minAttack, ToolPartHelper.maxAttack);
            toolTip.add(
                    StringHelper.getLocalizedString("tictooltips.tool.basedamage") + baseDamageColor
                            + StringHelper.getDamageString(baseDamage));
        }
        if (sprintDamage != damage) {
            String sprintDamageColor = ColorHelper.getRelativeColor(
                    ToolHelper.getRawDamage(tool, toolTag) + stoneboundDamage + (sprintDamage - damage),
                    ToolPartHelper.minAttack,
                    ToolPartHelper.maxAttack);
            toolTip.add(
                    EnumChatFormatting.DARK_GRAY + "- "
                            + StatCollector.translateToLocal("tictooltips.tool.sprint.damage")
                            + sprintDamageColor
                            + StringHelper.getDamageString(sprintDamage));
        }
        if (stoneboundDamage != 0) {
            EnumChatFormatting textColor = stoneboundDamage > 0 ? EnumChatFormatting.DARK_GREEN
                    : EnumChatFormatting.DARK_RED;
            String bonusOrLoss = (stoneboundDamage > 0 ? StringHelper.getLocalizedString("gui.toolstation4")
                    : StringHelper.getLocalizedString("gui.toolstation5")) + textColor;
            String shoddinessBonusOrLossTitle = StatCollector.translateToLocalFormatted(
                    "tictooltips.tool." + shoddinessCode + "." + bonusOrLossCode,
                    shoddinessType,
                    bonusOrLoss);
            String maxString = "";
            if (MathHelper.equals(stoneboundDamage, maxStoneboundDamage)) bonusOrLoss += EnumChatFormatting.BOLD;
            else maxString = EnumChatFormatting.RESET + " "
                    + EnumChatFormatting.DARK_GRAY
                    + "["
                    + StatCollector.translateToLocal("tictooltips.maximum")
                    + ": "
                    + StringHelper.getDamageNumberString((int) maxStoneboundDamage)
                    + EnumChatFormatting.RESET
                    + EnumChatFormatting.DARK_GRAY
                    + "]";
            toolTip.add(
                    EnumChatFormatting.DARK_GRAY + "- "
                            + shoddinessBonusOrLossTitle
                            + StringHelper.getDamageString((int) stoneboundDamage)
                            + maxString);
        } else if (!MathHelper.equals(maxStoneboundDamage, 0f)
                && !MathHelper.equals(stoneboundDamage, maxStoneboundDamage)) {
                    String bonusOrLoss = maxStoneboundDamage > 0
                            ? StringHelper.getLocalizedString("gui.toolstation4") + EnumChatFormatting.DARK_GREEN
                            : StringHelper.getLocalizedString("gui.toolstation5") + EnumChatFormatting.DARK_RED;
                    String maxShoddinessBonusOrLossTitle = StatCollector.translateToLocalFormatted(
                            "tictooltips.tool.max." + shoddinessCode + "." + bonusOrLossCode,
                            StatCollector.translateToLocal("tictooltips.maximum"),
                            shoddinessType,
                            bonusOrLoss);
                    toolTip.add(
                            EnumChatFormatting.DARK_GRAY + "- "
                                    + maxShoddinessBonusOrLossTitle
                                    + StringHelper.getDamageString((int) maxStoneboundDamage));
                }
        if (smiteDamageRange[1] != 0 && smiteDamageRange[0] != 0) {
            EnumChatFormatting textColor = smiteDamageRange[0] >= 0 ? EnumChatFormatting.DARK_GREEN
                    : EnumChatFormatting.DARK_RED;
            String bonusOrLoss = (smiteDamageRange[0] >= 0 ? StringHelper.getLocalizedString("gui.toolstation4")
                    : StringHelper.getLocalizedString("gui.toolstation5"));
            bonusOrLoss = bonusOrLoss.substring(0, bonusOrLoss.length() - 2);
            toolTip.add(
                    EnumChatFormatting.DARK_GRAY + "- "
                            + StatCollector.translateToLocalFormatted("tictooltips.tool.bonus.vs.undead", bonusOrLoss)
                            + textColor
                            + StringHelper.getDamageNumberString(smiteDamageRange[0])
                            + "-"
                            + StringHelper.getDamageString(smiteDamageRange[1]));
        }
        if (antiSpiderDamageRange[1] != 0 && antiSpiderDamageRange[0] != 0) {
            EnumChatFormatting textColor = antiSpiderDamageRange[0] >= 0 ? EnumChatFormatting.DARK_GREEN
                    : EnumChatFormatting.DARK_RED;
            String bonusOrLoss = (antiSpiderDamageRange[0] >= 0 ? StringHelper.getLocalizedString("gui.toolstation4")
                    : StringHelper.getLocalizedString("gui.toolstation5"));
            bonusOrLoss = bonusOrLoss.substring(0, bonusOrLoss.length() - 2);
            toolTip.add(
                    EnumChatFormatting.DARK_GRAY + "- "
                            + StatCollector.translateToLocalFormatted("tictooltips.tool.bonus.vs.spiders", bonusOrLoss)
                            + textColor
                            + StringHelper.getDamageNumberString(antiSpiderDamageRange[0])
                            + "-"
                            + StringHelper.getDamageString(antiSpiderDamageRange[1]));
        }
        if (burnDuration != 0) {
            EnumChatFormatting textColor = EnumChatFormatting.DARK_RED;
            toolTip.add(
                    EnumChatFormatting.DARK_GRAY + "- "
                            + StatCollector.translateToLocal("tictooltips.tool.burn.duration")
                            + textColor
                            + StringHelper.getDurationString(burnDuration));
        }
        if (chanceToBehead != 0) {
            toolTip.add(
                    EnumChatFormatting.DARK_GRAY + "- "
                            + StatCollector.translateToLocal("tictooltips.tool.chance.to.behead")
                            + ColorHelper.getRelativeColor(chanceToBehead, 0, 1)
                            + StringHelper.getPercentageString(chanceToBehead));
        }
        if (knockback != 0) {
            toolTip.add(
                    EnumChatFormatting.DARK_GRAY + "- "
                            + StatCollector.translateToLocal("tictooltips.tool.knockback")
                            + ColorHelper.getRelativeColor(knockback, 0.0f, 3.0f)
                            + StringHelper.getKnockbackString(knockback));
        }
    ...
    public List<String> getHarvestTooltip(ToolCore tool, NBTTagCompound toolTag) {
        ...
        toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation14") + ToolPartHelper.getMiningSpeedString(mineSpeed));
        if (stoneboundSpeed != 0) {
            EnumChatFormatting textColor = stoneboundSpeed > 0 ? EnumChatFormatting.DARK_GREEN
                    : EnumChatFormatting.DARK_RED;
            String bonusOrLoss = (stoneboundSpeed > 0 ? StringHelper.getLocalizedString("gui.toolstation4")
                    : StringHelper.getLocalizedString("gui.toolstation5")) + textColor;
            String shoddinessBonusOrLossTitle = StatCollector.translateToLocalFormatted(
                    "tictooltips.tool." + shoddinessCode + "." + bonusOrLossCode,
                    shoddinessType,
            ...
            if (MathHelper.equals(stoneboundSpeed, maxStoneboundSpeed))
                shoddinessBonusOrLossTitle += EnumChatFormatting.BOLD;
                    ...
                    + StatCollector.translateToLocal("tictooltips.maximum")
                    + ": "
                    + StringHelper.getSpeedString((int) (maxStoneboundSpeed * 100f))
                    + EnumChatFormatting.RESET
                    + EnumChatFormatting.DARK_GRAY
                    + "]";
            toolTip.add(
                    EnumChatFormatting.DARK_GRAY + "- "
                            + shoddinessBonusOrLossTitle
                            + StringHelper.getSpeedString((int) (stoneboundSpeed * 100f))
                            + maxString);
        } else
            if (!MathHelper.equals(maxStoneboundSpeed, 0f) && !MathHelper.equals(stoneboundSpeed, maxStoneboundSpeed)) {
                String bonusOrLoss = maxStoneboundSpeed > 0
                        ? StringHelper.getLocalizedString("gui.toolstation4") + EnumChatFormatting.DARK_GREEN
                        : StringHelper.getLocalizedString("gui.toolstation5") + EnumChatFormatting.DARK_RED;
                String maxShoddinessBonusOrLossTitle = StatCollector.translateToLocalFormatted(
                        "tictooltips.tool.max." + shoddinessCode + "." + bonusOrLossCode,
    ...
    public List<String> getDualHarvestTooltip(ToolCore tool, NBTTagCompound toolTag) {
        ...
        toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation12") + ToolPartHelper.getMiningSpeedString(mineSpeed1)
                        + EnumChatFormatting.RESET
                        + EnumChatFormatting.GRAY
                        + ", "
                        + ToolPartHelper.getMiningSpeedString(mineSpeed2));
        if (stoneboundSpeed != 0) {
            EnumChatFormatting textColor = stoneboundSpeed > 0 ? EnumChatFormatting.DARK_GREEN
                    : EnumChatFormatting.DARK_RED;
            String bonusOrLoss = (stoneboundSpeed > 0 ? StringHelper.getLocalizedString("gui.toolstation4")
        ...
        toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation13") + " "
                        + ToolPartHelper.getHarvestLevelString(harvestLevel1)
                        + EnumChatFormatting.RESET
                        + EnumChatFormatting.GRAY
                        + ", "
                        + ToolPartHelper.getHarvestLevelString(harvestLevel2));
    ...
    public List<String> getShoddinessTooltip(ToolCore tool, NBTTagCompound toolTag) {
        ...
        if (isShoddy) {
            ...
            if (StatCollector.canTranslate("tictooltips.tool." + shoddinessCode + ".modifier"))
                shoddinessModifierTitle = StatCollector
                        .translateToLocal("tictooltips.tool." + shoddinessCode + ".modifier");
            else {
                String shoddinessType = StringHelper.getShoddinessTypeString(shoddiness);
                shoddinessModifierTitle = StatCollector
                        .translateToLocalFormatted("tictooltips.tool.shoddiness.modifier", shoddinessType);
            }
    ...
    public List<String> getBowTooltip(ToolCore tool, NBTTagCompound toolTag, ItemStack itemStack) {
        ...
        if (itemStack != null && tool instanceof ProjectileWeapon) {
            ...
            ItemStack currentAmmo = projectileWeapon
                    .searchForAmmo(FMLClientHandler.instance().getClientPlayerEntity(), itemStack);
            if (currentAmmo != null) {
                if (ToolHelper.hasToolTag(currentAmmo) && currentAmmo.getItem() instanceof ToolCore) {
                    toolTip.addAll(
                            getWeaponTooltip(
                                    (ToolCore) currentAmmo.getItem(),
                                    ToolHelper.getToolTag(currentAmmo),
                                    projectileWeapon.getProjectileSpeed(itemStack)));

                    float[] critDamageRange = ToolHelper.getCriticalDamageRange(
                            (ToolCore) currentAmmo.getItem(),
                            ToolHelper.getToolTag(currentAmmo),
                            projectileWeapon.getProjectileSpeed(itemStack));
                    if (critDamageRange[1] != 0) {
                        EnumChatFormatting textColor = critDamageRange[0] >= 0 ? EnumChatFormatting.DARK_GREEN
                                : EnumChatFormatting.DARK_RED;
                        String bonusOrLoss = (critDamageRange[0] >= 0
                                ? StringHelper.getLocalizedString("gui.toolstation4")
                                : StringHelper.getLocalizedString("gui.toolstation5"));
                        bonusOrLoss = bonusOrLoss.substring(0, bonusOrLoss.length() - 2);
                        toolTip.add(
                                EnumChatFormatting.DARK_GRAY + "- "
                                        + StatCollector
                                                .translateToLocalFormatted("tictooltips.tool.crit.damage", bonusOrLoss)
                                        + textColor
                                        + StringHelper.getDamageNumberString(critDamageRange[0])
                                        + "-"
                                        + StringHelper.getDamageString(critDamageRange[1]));
                    }
                } else if (currentAmmo.getItem() == Items.arrow) {
                    int damage = (int) (2 * projectileWeapon.getProjectileSpeed(itemStack));
                    toolTip.add(
                            StringHelper.getLocalizedString("gui.toolstation3") + ColorHelper
                                    .getRelativeColor(damage, ToolPartHelper.minAttack, ToolPartHelper.maxAttack)
                                    + StringHelper.getDamageString(damage));
                }
            }
        }
        ...
        toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation6") + ToolPartHelper.getBowDrawSpeedString(drawSpeed));
        if (tool instanceof Crossbow || tool instanceof LongBow) toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation7")
                        + ToolPartHelper.getArrowSpeedString(arrowSpeed, 1.5f));
        else toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation7") + ToolPartHelper.getArrowSpeedString(arrowSpeed));
    ...
    public List<String> getUtilityTooltip(ToolCore tool, NBTTagCompound toolTag) {
        ...
        toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation16") + ToolPartHelper.getMiningSpeedString(mineSpeed));
    ...
    public List<String> getProjectileTooltip(ToolCore tool, NBTTagCompound toolTag) {
        ...
        toolTip.add(
                StringHelper.getLocalizedString("gui.toolstation22")
                        + ToolPartHelper.getBreakChanceString(breakChance));
    ...
    public List<String> getModifiersTooltip(ToolCore tool, NBTTagCompound toolTag) {
        ...
        while (toolTag.hasKey("ModifierTip" + tipNum)) {
            ...
            if (StatCollector.canTranslate(locString)) {
                ...
                // re-add the X/Y
                if (bracket > 0) tipName += " " + toolTag.getString("ModifierTip" + tipNum).substring(bracket);
            }
            if (!tipName.trim().equals("")) modifierToolTips.add(EnumChatFormatting.DARK_GRAY + "- " + tipName);
            tipNum++;
        }
    ...
    public List<String> getRepairTooltip(ToolCore tool, NBTTagCompound toolTag) {
        ...
        if (repairMat != null) {
            toolTip.add(
                    StatCollector.translateToLocal("tictooltips.tool.repair.material") + repairMat.style()
                            + CompatibilityHelper.getLocalizedName(repairMat));
        }
    ...
    private List<String> getToolStatsTooltip(ItemStack itemStack) {
        ...
        if (ToolHelper.hasAmmoCount(tool)) toolTip.addAll(getAmmoCountTooltip((IAmmo) tool, toolTag, itemStack));
        ...
        if (ToolHelper.isBow(tool)) toolTip.addAll(getBowTooltip(tool, toolTag, itemStack));
        else if (ToolHelper.isWeapon(tool)) toolTip.addAll(getWeaponTooltip(tool, toolTag));

        if (ToolHelper.isThrown(tool)) toolTip.addAll(getThrownTooltip(tool, toolTag));

        if (ToolHelper.isAmmo(tool)) toolTip.addAll(getAmmoTooltip(tool, toolTag));

        if (ToolHelper.isProjectile(tool)) toolTip.addAll(getProjectileTooltip(tool, toolTag));

        if (ToolHelper.isDualHarvestTool(tool)) toolTip.addAll(getDualHarvestTooltip(tool, toolTag));
        else if (ToolHelper.isHarvestTool(tool)) toolTip.addAll(getHarvestTooltip(tool, toolTag));
        else if (ToolHelper.isUtilityTool(tool)) toolTip.addAll(getUtilityTooltip(tool, toolTag));
